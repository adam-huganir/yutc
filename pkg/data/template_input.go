...
func NewTemplateInput(name string, isCommon bool, opts ...FileEntryOption) *TemplateInput {
	fe := NewFileEntry(name, opts...)
...
func (ti *TemplateInput) CollectContainerChildren() error {
	...
	case SourceKindFile:
		paths, err := WalkDir(ti.FileEntry, ti.logger)
		if err != nil {
			return err
		}
		for _, p := range paths {
			...
			child := NewTemplateInput(p, ti.IsCommon, WithSource(SourceKindFile))
